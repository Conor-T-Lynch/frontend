...
//Test for article creation and search functionality
describe('Article Creation and Search', () => {
  //Creates a new article and searching fir it
  it('should create a new article and search for it', () => {
    ...
    cy.get('table tbody')
      ...
      .should('exist');
    //Locate the search input field, make sure its visable, type the article title into the search input field
    cy.get('#searchInput')
      .should('be.visible')
      .type(articleData.title);
    //Submit the form
    cy.get('form.form-inline').submit();
    //Assert that search results contain the article title
    cy.contains(articleData.title).should('exist');
    //Assert that search results contain the article author
    cy.contains(articleData.author).should('exist');
    //Assert that search results contain the article content
